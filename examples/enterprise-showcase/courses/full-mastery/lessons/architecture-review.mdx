---
title:
  en: 'Architecture Review'
  es: 'Architecture Review (ES)'
description:
  en: 'Description for Architecture Review'
  es: 'Descripción para Architecture Review'
duration: '10 minutes'
---

# Architecture Review

<Callout type="info">
  Welcome to the new lesson: **Architecture Review**!
</Callout>

<Paragraph i18n="intro">
  <en>
    This is an auto-generated lesson content.
  </en>
  <es>
    Este es el contenido autogenerado de la lección.
  </es>
</Paragraph>

<Quiz
  id="architecture-review-quiz"
  questions={[
    {
      id: "q1",
      type: "multiple-choice",
      question: "Is this lesson auto-generated?",
      options: [
        { id: "a", label: "Yes" },
        { id: "b", label: "No" }
      ],
      correctAnswer: "a",
      points: 10
    }
  ]}
/>

<Progress value={50} showPercentage label="Lesson Progress" />
